...
class Buffer {
	...
	public Buffer(String fileName) {
		try {
			...
			throw new FatalMappingError("Could not open file " + fileName);
		}
	}
	...
	protected void Close() {
		if (file != null) {
			try {
				...
				throw new FatalMappingError(e.getMessage());
			}
		}
	}
	...
	public void setPos(int value) {
		...
		if (value < 0 || value > fileLen) {
			throw new FatalMappingError("buffer out of bounds access, position: " + value);
		}
	...
	private int ReadNextStreamChunk() {
		...
		catch (IOException ioex) { throw new FatalMappingError(ioex.getMessage()); }
...
public class Scanner {
	...
	static {
		...
		for (int i = 48; i <= 49; ++i) start.set(i, 2);
		for (int i = 51; i <= 57; ++i) start.set(i, 2);
		for (int i = 0; i <= 9; ++i) start.set(i, 3);
		for (int i = 11; i <= 12; ++i) start.set(i, 3);
		for (int i = 14; i <= 33; ++i) start.set(i, 3);
		for (int i = 35; i <= 35; ++i) start.set(i, 3);
		for (int i = 37; i <= 38; ++i) start.set(i, 3);
		for (int i = 42; i <= 44; ++i) start.set(i, 3);
		for (int i = 46; i <= 47; ++i) start.set(i, 3);
		for (int i = 58; i <= 64; ++i) start.set(i, 3);
		for (int i = 91; i <= 91; ++i) start.set(i, 3);
		for (int i = 93; i <= 94; ++i) start.set(i, 3);
		for (int i = 96; i <= 96; ++i) start.set(i, 3);
		for (int i = 123; i <= 65535; ++i) start.set(i, 3);
		start.set(50, 22); 
		start.set(34, 23); 
		start.set(40, 19); 
		start.set(41, 20); 
		...
		literals.put("qreg", new Integer(4));
		literals.put("creg", new Integer(5));
		literals.put("x", new Integer(6));
		literals.put("rz", new Integer(7));
		literals.put("sx", new Integer(8));
		literals.put("cx", new Integer(9));
		literals.put("swap", new Integer(10));
		literals.put("barrier", new Integer(11));
		literals.put("measure", new Integer(12));
		literals.put("OPENQASM", new Integer(13));
		literals.put("include", new Integer(15));
	...
	void Init () {
		...
		if (ch == 0xEF) { // check optional byte order mark for UTF-8
			...
			if (ch1 != 0xBB || ch2 != 0xBF) {
				throw new FatalMappingError("Illegal byte order mark at start of file");
			}
	...
	Token NextToken() {
		...
		loop: for (;;) {
			switch (state) {
				...
				case 3:
					{t.kind = 3; t.val = new String(tval, 0, tlen); CheckLiteral(); return t;}
				case 4:
					if (ch == '0') {AddCh(); state = 5; break;}
					else {state = 0; break;}
				case 5:
				...
				case 10:
					if (ch == 'b') {AddCh(); state = 11; break;}
					else {state = 0; break;}
				case 11:
					if (ch == '1') {AddCh(); state = 12; break;}
					else {state = 0; break;}
				case 12:
